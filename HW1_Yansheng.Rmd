...
From the formula sheet for linear combinations,
\[
E\!\left(\sum_{i=1}^n a_i Y_i\right)
= \sum_{i=1}^n a_i E(Y_i).
\]

Since $E(Y_i)=\beta_1 X_i$,
\[
E(\hat\beta_1)
...
Using the variance formula for linear combinations and $Var(Y_i)=\sigma^2$,
\[
V(\hat\beta_1)
= \sum_{i=1}^n a_i^2 \sigma^2
= \sigma^2 \sum_{i=1}^n
\frac{X_i^2}{\left(\sum_{j=1}^n X_j^2\right)^2}
= \frac{\sigma^2}{\sum_{i=1}^n X_i^2}.
\]

Since $\hat\beta_1$ is a linear combination of normally distributed $Y_i$’s,
\[
\hat\beta_1 \sim
...
= \sum_{i=1}^n c_i Y_i,
\qquad
c_i = \frac{1}{\sum_{j=1}^n X_j}.
\]

Using the expectation formula for linear combinations,
\[
E(\tilde\beta_1)
= \sum_{i=1}^n c_i E(Y_i)
= \frac{1}{\sum_{j=1}^n X_j}
\sum_{i=1}^n \beta_1 X_i
= \beta_1.
\]

Using the variance formula from the formula sheet,
\[
V(\tilde\beta_1)
= \sum_{i=1}^n c_i^2 Var(Y_i)
= \sum_{i=1}^n
\left(\frac{1}{\sum_{j=1}^n X_j}\right)^2 \sigma^2
...
\[
\tilde\beta_1 \sim
N\!\left(\beta_1,\ \frac{n\sigma^2}{\left(\sum_{i=1}^n X_i\right)^2}\right).
\]
...
\qquad
\frac{1}{V(\tilde\beta_1)}
= \frac{\left(\sum_{i=1}^n X_i\right)^2}{n\sigma^2}.
\]
...
Using the identity from the formula sheet,
\[
